...
class BLEMaster {
    ...
    startListener(profile_object, response_callback) {
        ...
        hmBle.mstOnPrepare((backend_response) => {
            ...
            // 3. [old] execute the response_callback with the profile pointer (profile_pid) as the argument 
            //    [new] returns status to the end user. (profile_pid) is now hidden from the end user interactions
            //    [new new] returns bool success and additional message with backend error codes
     ...
     * @example
     * .quit(); // example: quit interaction with a connected device and clean up
     */
    ...
    /**
     * @deprecated This method is deprecated and will be removed in the future. Please use quit() instead.
     */
    stop() {
        debugLog(3, ERR.DEPRECATED, "Please use quit() instead.");
        this.quit();
    }
...
class Read {
     ...
     * const desc = .read.descriptor('char_uuid', 'desc_uuid');
